<?php
// variable list for about page
$aboutVar = array(
    "socials" => array(
        "facebook" => "https://www.facebook.com/nick.oost.5/",
        "twitter" => "https://twitter.com/nick_oost",
        "linkedin" => "https://www.linkedin.com/in/nick-oost-6b1b3b1b3/",
        "instagram" => "https://www.instagram.com/nick_oost/"
    ),
    "skills" => array(
        "html" => "90%",
        "css" => "80%",
        "javascript" => "75%",
        "php" => "75%",
        "sql" => "80%"
    ),
    "projects" => array(
        "project1" => array(
            "title" => "Project 1",
            "description" =>
                "This is project 1
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "project2" => array(
            "title" => "Project 2",
            "description" =>
                "This is project 2
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "project3" => array(
            "title" => "Project 3",
            "description" =>
                "This is project 3
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "project4" => array(
            "title" => "Project 4",
            "description" =>
                "This is project 4
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "project5" => array(
            "title" => "Project 5",
            "description" =>
                "This is project 5
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "project6" => array(
            "title" => "Project 6",
            "description" =>
                "This is project 6
                <br>
                This is a project about a website that I created for a client. I created the website with HTML, CSS, JavaScript and PHP.
                <br>
                The website is a website where you can order a product and the order will be send to the client by email.
                <br>
                The website is not online yet.
            ",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "education" => array(
        "education1" => array(
            "title" => "Education 1",
            "description" => "This is education 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "education2" => array(
            "title" => "Education 2",
            "description" => "This is education 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "education3" => array(
            "title" => "Education 3",
            "description" => "This is education 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "hobbies" => array(
        "hobby1" => array(
            "title" => "Hobby 1",
            "description" => "This is hobby 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "hobby2" => array(
            "title" => "Hobby 2",
            "description" => "This is hobby 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "hobby3" => array(
            "title" => "Hobby 3",
            "description" => "This is hobby 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "movies" => array(
        "movie1" => array(
            "title" => "Movie 1",
            "description" => "This is movie 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "movie2" => array(
            "title" => "Movie 2",
            "description" => "This is movie 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "movie3" => array(
            "title" => "Movie 3",
            "description" => "This is movie 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "series" => array(
        "serie1" => array(
            "title" => "Serie 1",
            "description" => "This is serie 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "serie2" => array(
            "title" => "Serie 2",
            "description" => "This is serie 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "serie3" => array(
            "title" => "Serie 3",
            "description" => "This is serie 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "games" => array(
        "game1" => array(
            "title" => "Game 1",
            "description" => "This is game 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "game2" => array(
            "title" => "Game 2",
            "description" => "This is game 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "game3" => array(
            "title" => "Game 3",
            "description" => "This is game 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "music" => array(
        "music1" => array(
            "title" => "Music 1",
            "description" => "This is music 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "music2" => array(
            "title" => "Music 2",
            "description" => "This is music 2",
            "image" => "../src/img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "music3" => array(
            "title" => "Music 3",
            "description" => "This is music 3",
            "image" => "../src/img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "books" => array(
        "book1" => array(
            "title" => "Book 1",
            "description" => "This is book 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "book2" => array(
            "title" => "Book 2",
            "description" => "This is book 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "book3" => array(
            "title" => "Book 3",
            "description" => "This is book 3",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
    "sports" => array(
        "sport1" => array(
            "title" => "Sport 1",
            "description" => "This is sport 1",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "sport2" => array(
            "title" => "Sport 2",
            "description" => "This is sport 2",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        ),
        "sport3" => array(
            "title" => "Sport 3",
            "description" => "This is sport",
            "image" => "../img/placeholder.png",
            "link" => "https://www.google.com"
        )
    ),
);
    ?>